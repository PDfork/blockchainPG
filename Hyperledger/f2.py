print "Starting Factory 2";
print "Launching services... please wait"
import requests;
import sys;
data ={"$class":"org.production.auction.Update", "updateproduction":0, "flag":0, "listing":"org.production.auction.ProductionListing#opt1", "producer":"org.production.auction.Producer#F2"};
...
#		else:   print "Network error. Check the configuration"
		...
		if (b==2): 
			...
			a=-2.5-l/2;			#local objetive function
			data['updateproduction']=a;
			data['flag']=2;
			print "Computing new X2 value"
			#print(data);
			...
			if pc == 200:
				print("Updated X2 correctly to ledger")
				a = requests.get("http://10.0.2.15:3000/api/ProductionListing");
				aj= a.json();
				Lambda = aj[0]['LAMBDA'];
				f = aj[0]['f'];
